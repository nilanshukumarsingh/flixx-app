...
async function fetchAPIData(endpoint) {
  ...
  const response = await fetch(
    `${API_URL}?endpoint=${encodeURIComponent(endpoint)}&language=en-US`,
  );
...
async function searchAPIData() {
  ...
  const response = await fetch(
    `${API_URL}?endpoint=${encodeURIComponent(`search/${global.search.type}`)}&language=en-US&query=${encodeURIComponent(global.search.term)}&page=${global.search.page}`,
  );
  ...
  if (!response.ok) {
    hideSpinner();
    throw new Error(data.error || "Failed to search API data");
  }
